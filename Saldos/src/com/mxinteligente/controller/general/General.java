...
public class General extends GenericForwardComposer implements
ListitemRenderer {
	
	protected DataBinder binder;
	private Window winGen, winRecarga;
	...
	private Button btnGuardar, btnAceptar;
	...
	public void onClick$btnCategoria()throws Exception{
		Map params = new HashMap();
		params.put("user", usr);

		winCategoria = (Window) Executions.createComponents("/miSaldo/Categoria.zul", null, params);
		winCategoria.doModal();
		btnGuardar = (Button) winCategoria.getFellow("btnGuardar", true);
		btnGuardar.addEventListener("onAcceptYesIngreso", new EventListener() {
			public void onEvent(Event event)throws Exception {
				...
				binder.loadAll();
				logger.info("Guardado el objeto de Ingreso");
				
			}
		});
		
		btnGuardar.addEventListener("onAcceptYesEgreso", new EventListener() {
			public void onEvent(Event event)throws Exception {
				totalDatos = generalServ.obtenTotalEgresos(usr) + generalServ.obtenTotalIngresos(usr);
				tipo.setSelectedIndex(0);
				pg.setTotalSize(totalDatos);
	...
	public void onClick$btnGasto()throws Exception{
		Map params = new HashMap();
		params.put("user", usr);

		winEgreso = (Window) Executions.createComponents("/miSaldo/Gasto.zul", null, params);
		winEgreso.doModal();
		btnGuardar = (Button) winEgreso.getFellow("btnGuardar", true);
		btnGuardar.addEventListener("onAccept", new EventListener() {
			public void onEvent(Event event) throws Exception {
				lblSaldo.setLabel("Saldo: $"+generalServ.calculaSaldo(usr).toString());
				totalDatos = generalServ.obtenTotalEgresos(usr) + generalServ.obtenTotalIngresos(usr);
	...
	public void onClick$btnTransferencia()throws Exception{
		Map params = new HashMap();
		params.put("user", usr);

		winEgreso = (Window) Executions.createComponents("/miSaldo/Transferencia.zul", null, params);
		winEgreso.doModal();
		btnGuardar = (Button) winEgreso.getFellow("btnGuardar", true);
		btnGuardar.addEventListener("onAccept", new EventListener() {
			public void onEvent(Event event) throws Exception {
	...
	public void onClick$btnRetiroFondos() throws SuspendNotAllowedException, InterruptedException{
		Map params = new HashMap();
		params.put("user", usr);

		winEgreso = (Window) Executions.createComponents("/miSaldo/Retiro.zul", null, params);
		winEgreso.doModal();
		btnGuardar = (Button) winEgreso.getFellow("btnGuardar", true);
		btnGuardar.addEventListener("onAccept", new EventListener() {
			public void onEvent(Event event) throws Exception {
	...
	public void onClick$btnRecargar() throws SuspendNotAllowedException, InterruptedException{
		Map params = new HashMap();
		params.put("user", usr);

		winRecarga = (Window) Executions.createComponents("/miSaldo/Recargar.zul", null, params);
		winRecarga.doModal();
		winRecarga.setClosable(true);
		btnAceptar = (Button) winRecarga.getFellow("btnAceptar", true);
		btnAceptar.addEventListener("onAccept", new EventListener() {
			public void onEvent(Event event) throws Exception {
				lblSaldo.setLabel("Saldo: $"+generalServ.calculaSaldo(usr).toString());
				totalDatos = generalServ.obtenTotalEgresos(usr) + generalServ.obtenTotalIngresos(usr);
				pg.setTotalSize(totalDatos);
	...
	public void render(Listitem item, Object data) throws Exception {
		...
		item.appendChild(new Listcell(String.format("%05d", ((BigInteger)d.get("folio")).intValue())));
		...
		item.appendChild(new Listcell(d.get("contraparte")!=null? d.get("contraparte") + "":"N/A"));
		
		item.appendChild(new Listcell((String)d.get("concepto")));
		String scantidad="";
